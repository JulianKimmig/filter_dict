...
def get_method_keywords(method,return_contains_args_kwargs=True):
    ...
    filter_keys = [param.name for param in sig.parameters.values() if param.kind == param.POSITIONAL_OR_KEYWORD or param.kind == param.KEYWORD_ONLY]
    if not return_contains_args_kwargs:
        return filter_keys
    return filter_keys, len([param.name for param in sig.parameters.values() if param.kind == param.VAR_POSITIONAL])>0, len([param.name for param in sig.parameters.values() if param.kind == param.VAR_KEYWORD])>0
